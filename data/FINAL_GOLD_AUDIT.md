...
| **GOLD_138** | `IOS_SOFTWARE_UPDATE`<br/>`AUTO_HANDLE` | Spanish-language inquiry regarding iPhone 6 Plus performance under iOS 11. | **REVIEWED / DOCUMENTED** | Query text: *"como es posible que un iphone 6 plus no funcione correctamente en ios 11?Un smartphone de 800€ con un rendimiento inadmisible"*. Under Section 2 & 3, non-English messages route to localization specialists. However, the query was originally classified by software symptoms. Flagged for candidate human review: modifying this record's gold labels would change the frozen N=50 input hash (`GOLD_138` is part of `frozen_eval_subset_n50.json`). Preserved with candid failure commentary in `REPORT.md` (Failure Mode 2) to maintain complete cryptographic hash auditability without requiring remote API key re-generation. |
...
| **GOLD_149** | `CUSTOMER_FEEDBACK_COMPLAINT`<br/>`ESCALATE` | How-to guidance on Mail app trash vs archive configuration. | **REVIEWED / DOCUMENTED** | Query text: *"how can I send emails to the trash now instead of the archived option in the Apple Mail App"*. Semantically a UI configuration query (`DEVICE_SETUP_AND_USAGE`). Originally tagged with high customer frustration boilerplate. Documented in audit log as an illustrative taxonomy boundary case where user confusion regarding deleted vs archived mail in iOS Mail is escalated to human support when multi-provider IMAP server settings are involved. |
